...
class Admin_controller extends MY_Controller
{
        ...
        if (valid::form()) {
            switch (data::post('button')) {
                ...
                case 'Get Last Batch of Errors':
                    ...
                    break;
                case 'Update NDCs':
                    item::csv('medicine', 'ndc', "\t");
                    break;
                case 'Update Prices':
                    item::csv('medicine', 'price');
                    break;
                case 'Update Images':
                    item::url('medicine', 'image');
                    break;
                case 'Update Imprints':
                    item::url('medicine', 'imprint');
                    break;
            }
    ...
    */
    ...
    function drugs_by_donee_state($year)
    {
        ...
        header('Content-Description: File Transfer');
        header('Content-Type: application/octet-stream');
        header('Content-Disposition: attachment; filename="' . "SIRUM drugs_by_donee_state $year.csv" . '"');
        header('Expires: 0');
        header('Cache-Control: must-revalidate');
        header('Pragma: public');
        header('Content-Length: ' . filesize($path));
