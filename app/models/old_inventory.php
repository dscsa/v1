...
*/
	...
	function setFields($data) {
		foreach ($data as $index => $value) {
			...
			if ($value == 'drug label name') //Polaris
				self::$bulk['name'] = $index;
	...
	{
		if($row > 500){
			return  self::$bulk['alerts'][] = array_merge($data, ['beyond #row limit, just reuplaod']);
		}
		...
		if ($row == 1) {//Column headings
			if(strpos(strtolower($data[0]),'donations report') !== false){
		...
		//at this point it's etiehr Pharmerica, or its V2/Polaris/Coleman and it has a tracking number
		...
		if ($ndc AND strlen($ndc) > 0 AND ! preg_match('/^[0-9-]+$/', $ndc))
		{
		...
		if (count($donations) == 0) {
			if(self::isPharmerica() OR self::isV2()){
				...
				if(self::isPharmerica()){ //Create fake Pharmerica
					...
					//Our shipment id had a unique identifier for donor/donee. 
				...
				$donations[] = $donation;
				$created_donation = ' - Donation created in DB with id '.$donation->donation_id.' and tracking # '.$donation->tracking_number;
		...
		self::$bulk['upload'][] =
                [
                        'row' => $row,
			'created_donation' => $created_donation,
