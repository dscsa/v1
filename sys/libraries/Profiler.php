...
class CI_Profiler {
	...
	protected function _compile_benchmarks()
	{
		...
		//Adam Additions
		$model_query_time = number_format($this->CI->db->benchmark,4);
		
		$total_exec_time = array_pop($profile);
		
		$controller_time = array_pop($profile);
		
		$view_time = array_pop($profile);
		
		$base_time = array_shift($profile);
				
		$controller_time -= ($model_query_time + $view_time);
		
		$profile['CI Base'] = $base_time.' ('.round($base_time/$total_exec_time*100).'%)';
		
		$profile['Query'] = $model_query_time.' ('.round($model_query_time/$total_exec_time*100).'%)';
		
		$profile['Controller'] = $controller_time.' ('.round($controller_time/$total_exec_time*100).'%)';;
		
		$profile['View'] = $view_time.' ('.round($view_time/$total_exec_time*100).'%)';;
		
		$profile['Total'] = $total_exec_time.' (100%)';
	...
	protected function _compile_queries()
	{
		...
		foreach ($dbs as $db)
		{
			...
			$output .= "\n";
			$output .= '<legend style="color:#0000FF;">&nbsp;&nbsp;'.$this->CI->lang->line('profiler_database').':&nbsp; '.$db->database.'&nbsp;&nbsp;&nbsp;'.$this->CI->lang->line('profiler_queries').': '.count($db->queries).'&nbsp;&nbsp;&nbsp;'.number_format(array_sum($db->query_times), 3).'&nbsp;seconds&nbsp;&nbsp;</legend>';
			$output .= "\n";
			$output .= "\n\n<table style='width:100%;'>\n";
			...
			else
			{
				foreach ($db->queries as $key => $val)
				{
					...
					$output .= "<tr><td style='padding:5px; vertical-align: top;width:1%;color:#900;font-weight:normal;word-wrap:nowrap; background-color:#ddd;'>".$time."&nbsp;&nbsp;</td><td style='padding:5px; color:#000;font-weight:normal;background-color:#ddd;'>".$val."</td></tr>\n";
				}
			}
